"""
FINd Image Hasher — Optimised Implementation
=============================================
This module provides an optimised implementation of the FINd perceptual image
hashing algorithm developed by Find Images Now (FIN). It generates a 256-bit
hash fingerprint from an image, enabling efficient similarity detection and
duplicate clustering at scale.

Optimisation:
    The original boxFilter implementation used four nested Python loops,
    identified as the primary computational bottleneck via profiling.
    This implementation replaces the loops with scipy.ndimage.uniform_filter,
    achieving a 3.76x speedup while preserving the original algorithm logic.

Known Limitations:
    - Only square images are supported. Non-square images will raise a
      ValueError due to the rows indexing bug preserved for fair comparison
      with the original implementation.
    - Hashes may differ by up to 32 bits from the original implementation
      due to differences in boundary handling between the original clipping
      approach and SciPy's reflect padding.

Usage:
    from fin.hasher import FINDHasher

    hasher = FINDHasher()
    hash1 = hasher.fromFile("image1.jpg")
    hash2 = hasher.fromFile("image2.jpg")
...
from fin.matrix import MatrixUtil
...
class FINDHasher:
	...
	def boxFilter(cls, input, output, rows, cols, rowWin, colWin):
		# raise error early before any computations for non-square images due to rows indexing bug
		if rows != cols:
			raise ValueError(
				f"Non-square images are not supported by the optimised boxFilter. "
				f"Image dimensions: {rows}x{cols}. "
				f"Please use the original FINd implementation for non-square images."
			)
		
		# reshape input using same indexing as original (rows x rows)
		input_2d = np.array(input).reshape(rows, rows)
		
		# replace four nested Python loops with scipy uniform_filter
		filtered = uniform_filter(input_2d, size=[rowWin, colWin])
		
		# copy result to output in one line
		output[:rows*cols] = filtered.flatten()
